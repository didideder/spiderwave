...
def sanitize(input_str):
    output = BeautifulSoup(input_str.strip(), convertEntities=BeautifulSoup.HTML_ENTITIES).contents[0]
    try:
        output = unicode(output).encode('utf-8')
    except UnicodeDecodeError:
        output = unicode(output_enc.decode('utf-8')).encode('utf-8')
    return output
...
def add_radio(name, city, country, stream_urls, genres, logo=None,
    ...
    for stream_url, br in stream_urls:
        stream_url = sanitize(stream_url)
